...
Color Reference. Each section has a "Run on first result" button (plugin.py wires
...
def build_enhance_sections(mode, sdxl_choices=None, lora_choices=None):
    ...
    with gr.Accordion("Enhancements (post-process)", open=False,
        ...
        gr.Markdown("Applied to the **first** result with each section's Run "
        ...
        with gr.Accordion("ADetailer — detail refine", open=False):
            with gr.Tab("Face"):
                ...
                c["adetf_run"] = gr.Button("Run face ADetailer on first result", size="sm")
            with gr.Tab("Body"):
                ...
                c["adetb_run"] = gr.Button("Run body ADetailer on first result", size="sm")
        ...
        with gr.Accordion("Face Swap", open=False):
            ...
            c["face_run"] = gr.Button("Run face swap on first result", size="sm")
            ...
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    c["body_lora_mult"] = gr.Textbox(label="LoRA multipliers",
                                                     placeholder="0.8, 1.0")
                    c["body_run"] = gr.Button("Run body swap on first result", size="sm")
            ...
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    c["color_denoise"] = gr.Slider(0.0, 1.0, value=0.6, step=0.05,
                                                   label="Denoise")
            c["color_run"] = gr.Button("Apply color reference on first result", size="sm")
    return c
